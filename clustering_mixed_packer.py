...
from utils.dataset_utils import get_train_list, get_test_list
...
train_list = get_train_list() + get_test_list()
...
def frequency_feature_vecter_computation(packer_name, packed_files):
    ...
    for packed_file in packed_files:
        packer_name_by_file, file_name = get_packer_name_and_filename(packed_file)
        packed_dot_file = os.path.join(data_folder_path, "asm_cfg", packer_name_by_file,
                                       "{}_model.dot".format(packed_file))
        oep_address = oep_dictionary_2[packed_file]
        print("packed_file = {}, oep_address = {}".format(packed_file, oep_address))
        preceding_oep, msg = get_preceding_oep(packed_dot_file, oep_address)
        if not preceding_oep:
            print("Packer: {}, file_name: {}, error: {}".format(packer_name_by_file, packed_file, msg))
            continue
        packed_graph = get_removed_backed_graph(packer_name_by_file, file_name)
        node_list_packed_file, node_labels_packed_file, original_labels_packed_file, _ = convert_graph_to_vector(
            packed_graph,
            address=preceding_oep)

        data[packed_file] = Counter(list(node_labels_packed_file.values()) + original_labels_packed_file)
        opcode_sequence[packed_file] = get_opcode_sequence(packed_graph, preceding_oep)
...
def standard_generation(packed_files, opcode_sequence, labels, X, merged_unique_labels, first_k=5):
    ...
    for idx in range(len(packed_files)):
        ...
        opcode_sequence_of_label[label].append(opcode_sequence[packed_files[idx]])
...
def construct_standard_vector(packer_name, packed_files):
    ...
    for packed_file in packed_files:
        ...
        try:
            feature_vector = get_feature_vector(data[packed_file], merged_unique_labels)

            print(feature_vector)
            X.append(feature_vector)
            packer_name_list.append(_)
            idx += 1
    ...
    while True:
        ...
        cluster_by_idx = {}
        for idx, packer_name_iter in enumerate(packer_name_list):
            print("packer name: {}, {}".format(packer_name_iter, labels[idx]))
            if not (labels[idx] in cluster_by_idx):
                cluster_by_idx[labels[idx]] = []
            cluster_by_idx[labels[idx]].append(packer_name_iter)

        not_cross = True

        nb_of_class = 0
        for key in cluster_by_idx.keys():
            if key == "-1" or key == -1:
                continue
            nb_of_class += 1
        for key, value in cluster_by_idx.items():
            if key == "-1" or key == -1:
                continue
            if len(set(value)) != 1:
                not_cross = False
                break
        print("eps = {}, not cross: {}, # of class = {}".format(eps_default, not_cross, nb_of_class))
    ...
    with open("configs/end_of_unpacking_sequence_mixpacker.txt", "a") as f:
...
def main():
    ...
    for packer_name, packed_files in train_of.items():
        packed_all += packed_files
    print(len(packed_all))
    standard_feature_vectors = construct_standard_vector("all", packed_all)
    # print(standard_feature_vectors)
    # with open("configs/standard_feature_vectors_mixed/{}.json".format(packer_name), "w") as outfile:
